...
Earlier quality-75 entries above describe historical scanner observations. Their eight reported luminance values and successful decode did not constitute a retained complete Epson quality-75 table fixture. During R10 hardware QA on September 20, the known quality-75 `SCAN0003.JPG` supplied both complete 64-coefficient tables, now registered with firmware and sample provenance in `tests/fixtures/epson_quantization.json`. The original and its derivative pass explicit quality-75 verification in normal and optimized Python. The verifier still reports quality as unverified when no quality is requested or a requested table set is unregistered. A controlled known quality-50 capture must still be rejected against quality 75 to close that hardware check; synthetic test JPEGs do not satisfy it. See the [acceptance record](qa/reliability-acceptance.md) for current evidence.
