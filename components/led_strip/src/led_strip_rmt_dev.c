...
static esp_err_t led_strip_rmt_refresh(led_strip_t *strip)
{
    ...
    // A previous cleanup failure must be resolved before enabling again.
    if (rmt_strip->disable_pending) {
        ESP_RETURN_ON_ERROR(rmt_disable(rmt_strip->rmt_chan), TAG, "retry RMT disable failed");
        rmt_strip->disable_pending = false;
    }
    ESP_RETURN_ON_ERROR(rmt_enable(rmt_strip->rmt_chan), TAG, "enable RMT channel failed");
    rmt_strip->disable_pending = true;
    ...
    if (result == ESP_OK) {
        // The gateway drives one pixel; never block its recovery loop indefinitely.
        result = rmt_tx_wait_all_done(rmt_strip->rmt_chan, 1000);
    }
    // Always restore INIT after successful enable, including transmit/wait faults.
    esp_err_t cleanup = rmt_disable(rmt_strip->rmt_chan);
    if (cleanup == ESP_OK) {
        rmt_strip->disable_pending = false;
    } else {
        ESP_LOGE(TAG, "RMT cleanup failed: %s", esp_err_to_name(cleanup));
    }
    // Preserve the operation failure; cleanup is logged and retried next call.
