...
typedef enum {
    ...
    TINYUSB_MSC_EVENT_IO_ERROR,               /*!< Synchronous medium I/O failed; never a mount completion. */
} tinyusb_msc_event_id_t;

/** Operation associated with TINYUSB_MSC_EVENT_IO_ERROR. */
typedef enum {
    TINYUSB_MSC_IO_READ,
    TINYUSB_MSC_IO_WRITE,
} tinyusb_msc_io_operation_t;
...
typedef struct {
    ...
    union {
        struct {
            uint8_t lun;
            tinyusb_msc_io_operation_t operation;
            esp_err_t error;
        } io_error;                          /*!< Valid only for IO_ERROR; copy before returning. */
        struct {
 ...
 *
 * @note IO_ERROR runs in the TinyUSB task. Record bounded state only; do not
 *       render, mount, or wait in that callback. Handle may be NULL for an invalid LUN.
 ...
 *
 * @note Stop/quiesce USB before deleting storage so no new callback can start.
 *       Active synchronous I/O also causes deletion to fail closed.
 *
 ...
 *      - ESP_ERR_INVALID_STATE if the driver is not installed, no storage exists, or synchronous I/O is active
