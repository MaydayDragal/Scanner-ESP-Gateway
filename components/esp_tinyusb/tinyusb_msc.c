...
/* Stack-local event, no allocation or queue. The application callback must
 * only record the error; it must not render, mount, or wait for another task. */
static void tinyusb_io_error_cb(uint8_t lun, tinyusb_msc_io_operation_t operation, esp_err_t error)
{
    MSC_ENTER_CRITICAL();
    tusb_msc_callback_t cb = p_msc_driver ? p_msc_driver->dynamic.event_cb : NULL;
    void *arg = p_msc_driver ? p_msc_driver->dynamic.event_arg : NULL;
    msc_storage_obj_t *storage = p_msc_driver && lun < TINYUSB_MSC_STORAGE_MAX_LUNS ?
                                 p_msc_driver->dynamic.storage[lun] : NULL;
    tinyusb_msc_event_t event = {
        .id = TINYUSB_MSC_EVENT_IO_ERROR,
        .mount_point = storage ? storage->mount_point : TINYUSB_MSC_STORAGE_MOUNT_USB,
...
static inline esp_err_t msc_storage_read_sector(uint8_t lun, uint32_t lba, uint32_t offset, size_t size, void *dest)
{
    ...
    if (found && storage != NULL) storage->active_io++;
    MSC_EXIT_CRITICAL();
...
esp_err_t tinyusb_msc_delete_storage(tinyusb_msc_storage_handle_t handle)
{
    ...
    MSC_CHECK_ON_CRITICAL(p_msc_driver != NULL, ESP_ERR_INVALID_STATE);
    MSC_CHECK_ON_CRITICAL(p_msc_driver->dynamic.lun_count > 0, ESP_ERR_INVALID_STATE);
    MSC_CHECK_ON_CRITICAL(storage->active_io == 0, ESP_ERR_INVALID_STATE);
    MSC_EXIT_CRITICAL();
...
int32_t tud_msc_write10_cb(uint8_t lun, uint32_t lba, uint32_t offset, uint8_t *buffer, uint32_t bufsize)
{
    ...
    esp_err_t err = bufsize > MSC_STORAGE_BUFFER_SIZE ? ESP_ERR_INVALID_SIZE :
                    msc_storage_write_sector(lun, lba, offset, bufsize, buffer);
    if (err != ESP_OK) {
        tinyusb_io_error_cb(lun, TINYUSB_MSC_IO_WRITE, err);
        return TUD_MSC_RET_ERROR;
    }
