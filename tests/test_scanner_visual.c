...
esp_err_t esp_lcd_panel_disp_on_off(esp_lcd_panel_handle_t a,bool on) {(void)a;if(!on) panel_off_calls++;if(fail_panel)return ESP_FAIL;panel_on=on;return ESP_OK;}
esp_err_t esp_lcd_panel_draw_bitmap(esp_lcd_panel_handle_t a,int b,int c,int d,int e,const void *f) {(void)a;(void)b;(void)c;(void)d;(void)e;(void)f;return ESP_OK;}
...
esp_err_t led_strip_refresh(led_strip_handle_t a) {(void)a;if(fail_refresh||fail_clear)return ESP_FAIL;led_on=led_buffer;return ESP_OK;}
